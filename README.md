...
### 步骤6： 搭建3D-ResNet(`model/UNet3DMMS`)

### 网络结构

```
编码器路径 (下采样)：
Input → Conv3D_Block → Pool1 → Conv3D_Block → Pool2 → ... → 瓶颈层

解码器路径 (上采样)：
瓶颈层 → Up_Block + 跳跃连接 → Conv3D_Block → ... → 输出层
...
1. **Conv3D_Block**：双卷积层+批量归一化+ReLU激活，可选残差连接
2. **Up_Block**：三线性插值上采样 + 卷积通道减半
3. **多尺度池化**：在不同维度组合使用(1,2,2)和(2,2,2)的池化核


### 步骤7： 数据集处理Dataset(`generators/image_label_generator`)

## 核心功能
为3D医学图像分割网络（UNet3DMMS）提供标准化的数据预处理和增强。训练集数据增强，验证集不增强

## 两个主要类
1. **Image_Label_train** - 训练数据集
   - 70%概率应用多种数据增强
   - 包括翻转、旋转、亮度调整、添加噪声
   - 图像和标签同步增强，确保一致性

2. **Image_Label_valid** - 验证数据集
   - 不进行任何数据增强
   - 保持原始数据用于准确评估

## 关键特点
- 处理3D体数据（.npy格式）
- 自动添加通道维度（[D,H,W]→[1,D,H,W]）
- 转换为PyTorch张量格式
- 支持批量训练和验证

## 作用
将原始医学图像转换为适合3D分割网络训练的标准化输入，提高模型泛化能力和鲁棒性。
