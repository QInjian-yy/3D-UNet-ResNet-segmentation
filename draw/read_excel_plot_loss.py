...
excel_names = []  # 初始化一个列表来存储Excel文件名
# 遍历文件列表，过滤出所有扩展名为.xlsx的文件
for i in files_list:
    if os.path.splitext(i)[1] == ".xlsx":
        excel_names.append(i)

# 初始化列表来存储epoch、训练损失和验证损失
epoch, loss_train, loss_valid = [], [], []
# 遍历每个Excel文件
for i in range(len(excel_names)):
    epoch.append(i + 1)  # 记录当前epoch（从1开始）
    excel_path = excel_folder + excel_names[i]  # 构建完整的Excel文件路径
    workbook = openpyxl.load_workbook(filename=excel_path)  # 打开Excel工作簿
    sheet = workbook['loss_train_%d' % (i + 1)]  # 获取指定名称的工作表（格式：'loss_train_epoch_number'）
    loss_train.append(sheet.cell(row=1, column=3).value)  # 从工作表中读取训练损失数据并添加到列表中
    loss_valid.append(sheet.cell(row=1, column=4).value)  # 从工作表中读取验证损失数据并添加到列表中
...
plt.ylim(0.2, 2.95)  # 设置y轴范围从0.2到2.95
