...
#此文件的主要作用是批量读取医学图像数据集中的 NIfTI 格式文件，提取关键信息（如体素尺寸、图像维度等），
# 并将这些信息整理保存到 Excel 表格中，方便后续分析和处理
...
for index in range(len(file_names)):
    ...
    # 从图像头信息中获取体素尺寸(空间分辨率)（单位通常为mm），保留前三个值（X,Y,Z方向）
    voxel = file.header.get_zooms()[:3]
    # 获取图像维度(体素数量)
    width, height, depth = file.shape[:3]
