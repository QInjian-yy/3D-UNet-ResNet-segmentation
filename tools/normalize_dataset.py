...
for idx in range(len(image_names)):
    ...
    q5 = np.quantile(image_array, 0.05)  # 计算5%分位数（排除最小5%的异常低值）
    q95 = np.quantile(image_array, 0.95)  # 计算95%分位数（排除最大5%的异常高值）

    image_array[image_array < q5] = q5  # 将小于5%分位数的值设为q5
    image_array[image_array > q95] = q95  # 将大于95%分位数的值设为q95

    image_array = (image_array - np.mean(image_array)) / np.std(image_array)  # Z-score标准化
